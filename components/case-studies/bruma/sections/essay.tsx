...
const BODY_TEXT = 'text-[clamp(1.0625rem,0.9vw+0.85rem,1.1875rem)] leading-[1.7] text-[#1D1B18]/80'
const LEDE = 'max-w-2xl text-[clamp(1.85rem,5.5vw,2.75rem)] leading-[1.16] tracking-[-0.01em] italic'
 ...
 * as their own grid sections. Paragraphs stay short and separated
 * (space-y is wider on mobile than desktop) so a narrow column never
 * reads as one dense block of text.
 */
export function BrumaEssay() {
  return (
    <article className="mx-auto max-w-[1400px] px-6 sm:px-10">
      {/* La premisa */}
      <section id="bruma-premisa" className="scroll-mt-24 border-t border-[#1D1B18]/12 py-16 sm:py-20">
        <Reveal variants={brumaFade}>
          <p className="text-[11px] tracking-[0.22em] uppercase text-[#4A3324]">{premisa.kicker}</p>
          <h2 className={`mt-5 ${LEDE}`} style={{ fontFamily: 'var(--font-bruma-display)' }}>
            {premisa.lede}
          </h2>
        </Reveal>
        <Reveal variants={brumaFade} delay={0.08} className="mt-10 max-w-[62ch] space-y-6 sm:space-y-5">
        ...
        <StaggerGroup className="mt-10 max-w-[62ch] space-y-6 sm:space-y-5" gap={0.06}>
      ...
      {/* Referencias */}
      <section id="bruma-referencias" className="scroll-mt-24 border-t border-[#1D1B18]/12 py-16 sm:py-20">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_300px] lg:gap-16">
          <div>
            ...
            <Reveal variants={brumaFade} delay={0.08} className="mt-10 max-w-[62ch] space-y-6 sm:space-y-5">
              {referencias.paragraphs.map((paragraph) => (
                <p key={paragraph} className={BODY_TEXT}>
                  {paragraph}
                </p>
              ))}
            ...
            <Reveal variants={brumaFade} delay={0.14} className="relative mt-10 -mx-6 aspect-[4/3] overflow-hidden sm:mx-0 sm:max-w-[62ch]">
      ...
      {/* El sistema */}
      <section id="bruma-sistema" className="scroll-mt-24 border-t border-[#1D1B18]/12 py-16 sm:py-20">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_300px] lg:gap-16">
          <div>
            <Reveal variants={brumaFade}>
              <p className="text-[11px] tracking-[0.22em] uppercase text-[#4A3324]">{sistema.kicker}</p>
              <h2 className={`mt-5 ${LEDE} max-w-xl`} style={{ fontFamily: 'var(--font-bruma-display)' }}>
                {sistema.lede}
              </h2>
            </Reveal>
            <Reveal variants={brumaFade} delay={0.08} className="mt-10 max-w-[62ch] space-y-6 sm:space-y-5">
              {sistema.paragraphs.map((paragraph) => (
                <p key={paragraph} className={BODY_TEXT}>
                  {paragraph}
                </p>
              ))}
              <p className={BODY_TEXT}>{sistema.componentes}</p>
            </Reveal>

            <Reveal variants={brumaFade} delay={0.14} className="mt-10 max-w-[62ch] border-t border-[#1D1B18]/12 pt-7">
              ...
              <p className="mt-7 text-[10px] tracking-[0.18em] text-[#4A3324] uppercase">{sistema.tipografia.body.name}</p>
      ...
      {/* La composición */}
      <section id="bruma-composicion" className="scroll-mt-24 border-t border-[#1D1B18]/12 py-16 sm:py-20">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_300px] lg:gap-16">
          <div>
            <Reveal variants={brumaFade}>
            ...
            <Reveal variants={brumaFade} delay={0.08} className="mt-10 max-w-[62ch] space-y-6 sm:space-y-5">
              {composicion.paragraphs.map((paragraph) => (
                <p key={paragraph} className={BODY_TEXT}>
                  {paragraph}
                </p>
              ))}
            ...
            <StaggerGroup className="mt-10 grid max-w-[62ch] grid-cols-1 gap-6 sm:grid-cols-3 sm:gap-3" gap={0.06}>
              {composicion.etapas.map((stage, index) => (
                <RevealItem key={stage.n} variants={brumaFade}>
                  <div
                    className="mx-auto max-w-[200px] border border-[#1D1B18]/15 bg-[#EDE8DC] sm:mx-0 sm:max-w-none"
                    style={{ aspectRatio: index === 0 ? '16/10' : index === 1 ? '4/3' : '9/16' }}
                  />
                  <p className="mt-3 text-center text-[11px] tracking-[0.1em] text-[#4A3324] uppercase sm:text-left">{stage.label}</p>
                  <p className="mt-1 max-w-[240px] text-center text-xs leading-relaxed text-[#1D1B18]/65 sm:mx-0 sm:max-w-none sm:text-left">{stage.nota}</p>
                </RevealItem>
              ))}
            </StaggerGroup>
          </div>
