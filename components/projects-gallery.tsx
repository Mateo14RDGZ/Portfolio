              ...
              <RevealItem key={project.slug} className="h-full min-w-0" variants={compactMotion ? stationaryCard : undefined}>
                <motion.article
                  ...
                  className={cn(
                    ...
                    project.slug === 'cimbra-estudio' && 'rounded-[2.7rem] border-[#cbdcd7] bg-[#edf2ec] text-[#17332f]',
                  )}
                >
                  ...
                  <div className="flex min-w-0 flex-1 flex-col p-5 sm:p-7">
                    ...
                    <p className={cn('mt-3 flex-1 text-sm leading-relaxed', project.slug === 'bruma-cafe' && 'text-[#1D1B18]/70', project.slug === 'astra' && 'text-[#10151B]/62', project.slug === 'cimbra-estudio' && 'text-[#17332f]/64')}>{project.description}</p>
                    <div className={cn('mt-6 flex items-center justify-between pt-5', project.slug === 'bruma-cafe' && 'border-t border-[#1D1B18]/18', project.slug === 'astra' && 'border-t border-[#10151B]/12', project.slug === 'cimbra-estudio' && 'border-t border-[#17332f]/15')}>
                      <span className={cn('flex items-center gap-2 text-[9px] tracking-[0.12em] uppercase', project.slug === 'bruma-cafe' && 'text-[#1D1B18]/58', project.slug === 'astra' && 'text-[#10151B]/55', project.slug === 'cimbra-estudio' && 'text-[#17332f]/55')}><Layers3 className={cn('size-3.5', project.slug === 'bruma-cafe' && 'text-[#4A3324]', project.slug === 'astra' && 'text-[#4F7FA0]', project.slug === 'cimbra-estudio' && 'text-[#5b8580]')} /> Landing completa</span>
                      <Link href={`/proyectos/${project.slug}`} onClick={() => track('projects_click', { project: project.slug })} className={cn('grid size-11 place-items-center transition-colors', project.slug === 'bruma-cafe' && 'rounded-full border border-[#1D1B18]/35 hover:bg-[#4A3324] hover:text-[#F3F0EA]', project.slug === 'astra' && 'border border-[#10151B] hover:bg-[#10151B] hover:text-[#F1F3F6]', project.slug === 'cimbra-estudio' && 'rounded-full bg-[#5b8580] text-white hover:bg-[#17332f]')} aria-label={`Abrir ${project.name}`}><ArrowUpRight className="size-4" /></Link>
                    </div>
                  </div>
                </motion.article>
              </RevealItem>
