              ...
              <RevealItem key={project.slug} className="h-full min-w-0" variants={compactMotion ? stationaryCard : undefined}>
                <motion.article
                  ...
                  className={cn(
                    ...
                    project.slug === 'bruma-cafe' && 'rounded-[2.7rem_0.45rem_2.7rem_0.45rem] border-[#4A3324]/12 bg-[#F3F0EA] text-[#1D1B18]',
                    project.slug === 'astra' && 'rounded-none border-[#10151B]/12 bg-[#F1F3F6] text-[#10151B]',
                    project.slug === 'cimbra-estudio' && 'rounded-[2.7rem] border-[#1C222B]/10 bg-[#ECEFF3] text-[#1C222B]',
                  )}
                >
                  <Link href={`/proyectos/${project.slug}`} className={cn('relative mx-4 mt-4 aspect-[1.25] overflow-hidden sm:mx-5 sm:mt-5 sm:aspect-[1.18]', project.slug === 'bruma-cafe' && 'rounded-[2rem_0.3rem_2rem_0.3rem]', project.slug === 'astra' && 'rounded-none', project.slug === 'cimbra-estudio' && 'rounded-[2rem]')} aria-label={`Ver landing conceptual ${project.name}`}>
                    ...
                    <span className={cn('absolute top-4 left-4 px-3 py-2 text-[8px] tracking-[0.14em] uppercase', project.slug === 'bruma-cafe' && 'rounded-full border border-white/30 bg-[#1D1B18]/75 text-[#F3F0EA]', project.slug === 'astra' && 'border border-[#10151B]/15 bg-[#F1F3F6]/90 text-[#10151B] backdrop-blur-md', project.slug === 'cimbra-estudio' && 'rounded-full bg-[#ECEFF3]/90 text-[#1C222B] shadow-[4px_4px_10px_rgba(28,34,43,0.14)]')}>Caso de diseño</span>
                  </Link>
                  <div className="flex min-w-0 flex-1 flex-col p-5 sm:p-7">
                    <p className={cn('text-[9px] tracking-[0.15em] uppercase', project.slug === 'bruma-cafe' && 'text-[#4A3324]', project.slug === 'astra' && 'text-[#4F7FA0]', project.slug === 'cimbra-estudio' && 'text-[#FF6B4A]')}>0{index + 2} / {project.category.split(' · ')[0]}</p>
                    <h3 className={cn('mt-4 w-full max-w-full min-w-0 [overflow-wrap:anywhere] text-[clamp(1.8rem,9vw,2.25rem)] leading-[0.98] tracking-[-0.05em] sm:text-3xl', project.slug === 'bruma-cafe' && 'font-semibold italic', project.slug === 'astra' && 'font-semibold uppercase', project.slug === 'cimbra-estudio' && 'font-bold')}>{project.name}</h3>
                    <p className={cn('mt-3 flex-1 text-sm leading-relaxed', project.slug === 'bruma-cafe' && 'text-[#1D1B18]/70', project.slug === 'astra' && 'text-[#10151B]/62', project.slug === 'cimbra-estudio' && 'text-[#1C222B]/62')}>{project.description}</p>
                    <div className={cn('mt-6 flex items-center justify-between pt-5', project.slug === 'bruma-cafe' && 'border-t border-[#1D1B18]/18', project.slug === 'astra' && 'border-t border-[#10151B]/12', project.slug === 'cimbra-estudio' && 'border-t border-[#1C222B]/10')}>
                      <span className={cn('flex items-center gap-2 text-[9px] tracking-[0.12em] uppercase', project.slug === 'bruma-cafe' && 'text-[#1D1B18]/58', project.slug === 'astra' && 'text-[#10151B]/55', project.slug === 'cimbra-estudio' && 'text-[#1C222B]/55')}><Layers3 className={cn('size-3.5', project.slug === 'bruma-cafe' && 'text-[#4A3324]', project.slug === 'astra' && 'text-[#4F7FA0]', project.slug === 'cimbra-estudio' && 'text-[#FF6B4A]')} /> Landing completa</span>
                      <Link href={`/proyectos/${project.slug}`} onClick={() => track('projects_click', { project: project.slug })} className={cn('grid size-11 place-items-center transition-colors', project.slug === 'bruma-cafe' && 'rounded-full border border-[#1D1B18]/35 hover:bg-[#4A3324] hover:text-[#F3F0EA]', project.slug === 'astra' && 'border border-[#10151B] hover:bg-[#10151B] hover:text-[#F1F3F6]', project.slug === 'cimbra-estudio' && 'rounded-full bg-[#FF6B4A] text-white hover:bg-[#1C222B]')} aria-label={`Abrir ${project.name}`}><ArrowUpRight className="size-4" /></Link>
                    </div>
                  </div>
                </motion.article>
              </RevealItem>
