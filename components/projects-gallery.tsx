          ...
          <StaggerGroup className="mt-8 grid grid-cols-1 items-stretch gap-4 lg:grid-cols-3" gap={compactMotion ? 0 : 0.08}>
            {CONCEPT_PROJECTS.map((project, index) => (
              <RevealItem key={project.slug} className="h-full min-w-0" variants={compactMotion ? stationaryCard : undefined}>
                ...
                >
                  <Link href={`/proyectos/${project.slug}`} className={cn('relative mx-4 mt-4 aspect-[1.25] overflow-hidden sm:mx-5 sm:mt-5 sm:aspect-[1.18]', project.slug === 'bruma-cafe' && 'rounded-[2rem_0.3rem_2rem_0.3rem]', project.slug === 'aster-automoviles' && 'rounded-none', project.slug === 'cimbra-estudio' && 'rounded-[2rem]')} aria-label={`Ver landing conceptual ${project.name}`}>
                    ...
                    <span className={cn('absolute top-4 left-4 px-3 py-2 text-[8px] tracking-[0.14em] uppercase', project.slug === 'bruma-cafe' && 'rounded-full border border-white/30 bg-[#302218]/75 text-[#fff7e8] [font-family:var(--font-fraunces),Georgia,serif]', project.slug === 'aster-automoviles' && 'bg-[#e7e3dc] text-[#242522] [font-family:var(--font-space-grotesk),Arial,sans-serif]', project.slug === 'cimbra-estudio' && 'rounded-full bg-[#edf2ec]/90 text-[#17332f] [font-family:var(--font-mazius-display),Georgia,serif]')}>Caso de diseño</span>
                  </Link>
                  <div className="flex min-w-0 flex-1 flex-col p-5 sm:p-7">
                    <p className={cn('text-[9px] tracking-[0.15em] uppercase', project.slug === 'bruma-cafe' && 'text-[#8a5a3b] [font-family:var(--font-fraunces),Georgia,serif]', project.slug === 'aster-automoviles' && 'text-[#9a5d3b] [font-family:var(--font-space-grotesk),Arial,sans-serif]', project.slug === 'cimbra-estudio' && 'text-[#5b8580] [font-family:var(--font-mazius-display),Georgia,serif]')}>0{index + 2} / {project.category.split(' · ')[0]}</p>
                    <h3 className={cn('mt-4 w-full max-w-full min-w-0 [overflow-wrap:anywhere] text-[clamp(1.8rem,9vw,2.25rem)] leading-[0.98] tracking-[-0.05em] sm:text-3xl', project.slug === 'bruma-cafe' && 'font-semibold italic', project.slug === 'aster-automoviles' && 'font-semibold uppercase', project.slug === 'cimbra-estudio' && 'font-bold')}>{project.name}</h3>
                    <p className={cn('mt-3 flex-1 text-sm leading-relaxed', project.slug === 'bruma-cafe' && 'text-[#302218]/70', project.slug === 'aster-automoviles' && 'text-[#242522]/62', project.slug === 'cimbra-estudio' && 'text-[#17332f]/64')}>{project.description}</p>
                    <div className={cn('mt-6 flex items-center justify-between pt-5', project.slug === 'bruma-cafe' && 'border-t border-[#302218]/18', project.slug === 'aster-automoviles' && 'border-t border-[#242522]', project.slug === 'cimbra-estudio' && 'border-t border-[#17332f]/15')}>
                      <span className={cn('flex items-center gap-2 text-[9px] tracking-[0.12em] uppercase', project.slug === 'bruma-cafe' && 'text-[#302218]/58 [font-family:var(--font-fraunces),Georgia,serif]', project.slug === 'aster-automoviles' && 'text-[#242522]/55 [font-family:var(--font-space-grotesk),Arial,sans-serif]', project.slug === 'cimbra-estudio' && 'text-[#17332f]/55 [font-family:var(--font-mazius-display),Georgia,serif]')}><Layers3 className={cn('size-3.5', project.slug === 'bruma-cafe' && 'text-[#b75632]', project.slug === 'aster-automoviles' && 'text-[#9a5d3b]', project.slug === 'cimbra-estudio' && 'text-[#5b8580]')} /> Landing completa</span>
