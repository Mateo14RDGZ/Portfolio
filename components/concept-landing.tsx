...
function OmbuLanding() {
  ...
  return (
    <main className="overflow-x-clip bg-[#edf0df] text-[#23201b] [font-family:var(--font-fraunces),Georgia,serif]">
      ...
      <section className="border-y border-[#23201b]/15 bg-[#e3ddc4] px-5 py-9 sm:px-8 lg:px-12">
        <Reveal className="mx-auto grid max-w-[1220px] grid-cols-3 gap-4 text-center sm:text-left">
          {stats.map(([value, label]) => (
            <div key={label}><b className="block text-[clamp(2.2rem,5vw,3.2rem)] leading-none tracking-[-0.05em]">{value}</b><p className="mt-2 text-xs leading-snug text-[#4e493e] sm:text-sm">{label}</p></div>
          ))}
        </Reveal>
      </section>

      <section id="origen" className="border-b border-[#23201b]/15 px-5 py-20 sm:px-8 sm:py-28 lg:px-12">
        <div className="mx-auto grid max-w-[1220px] gap-12 lg:grid-cols-[0.85fr_0.62fr_0.53fr] lg:items-end">
      ...
      <section id="visita" className="bg-[#29261f] px-5 py-20 text-[#edf0df] sm:px-8 sm:py-28 lg:px-12"><div className="mx-auto grid max-w-[1220px] gap-10 lg:grid-cols-[1fr_auto] lg:items-end"><Reveal><p className="text-[11px] tracking-[0.18em] uppercase text-[#c9c2a4]">Ombú Café de Especialidad</p><h2 className="mt-5 max-w-3xl text-[clamp(3.7rem,6.6vw,7.2rem)] leading-[0.8] tracking-[-0.08em]">Pasá cuando necesites una pausa.</h2></Reveal><Reveal delay={0.08} className="space-y-3 text-base text-[#edf0df]/75"><p className="flex gap-3"><MapPin className="mt-0.5 size-4" />Ciudad Vieja, Montevideo</p><p>Lunes a sábado, 08:30 - 18:00</p><Link href="/#contact" className="mt-6 inline-flex min-h-12 items-center gap-3 border border-[#edf0df]/45 px-5 text-[#edf0df]">Quiero una web así <ArrowUpRight className="size-4" /></Link></Reveal></div><div className="mx-auto mt-14 max-w-[1220px] border-t border-[#edf0df]/20 pt-5"><ProjectBack /></div></section>
    </main>
  )
}
...
function CimbraLanding() {
  ...
  const stats = [
    ['8 años', 'formando parte del barrio'],
    ['1.200+', 'clases dictadas por mes'],
  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-[#f8f9f6] text-[#14212e] [font-family:var(--font-nunito-sans),Arial,sans-serif]">
      ...
      <section className="border-y border-[#14212e]/12 bg-[#eef0e8] px-5 py-9 sm:px-8 lg:px-10"><Reveal className="mx-auto grid max-w-[1320px] grid-cols-3 gap-4 text-center sm:text-left">{stats.map(([value, label]) => <div key={label}><b className="block text-[clamp(1.7rem,4.2vw,2.6rem)] leading-none font-black tracking-[-0.04em] text-[#2364e6]">{value}</b><p className="mt-2 text-xs leading-snug opacity-70 sm:text-sm">{label}</p></div>)}</Reveal></section>
      ...
      <section className="border-t border-[#14212e]/12 px-5 py-16 sm:px-8 sm:py-24 lg:px-10"><div className="mx-auto grid max-w-[1180px] gap-12 lg:grid-cols-[0.75fr_1.25fr]"><Reveal><h2 className="text-[clamp(2.6rem,4.6vw,4.2rem)] leading-[.88] font-black tracking-[-.07em]">Preguntas antes de tu primera clase.</h2></Reveal><div>{faqs.map(([question, answer]) => <details key={question} className="group border-t border-[#14212e]/15 py-5 last:border-b"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-xl font-bold"><span>{question}</span><ChevronDown className="size-5 transition-transform duration-300 group-open:rotate-180" /></summary><p className="max-w-xl pt-4 leading-relaxed opacity-70">{answer}</p></details>)}</div></div><div className="mx-auto mt-14 max-w-[1180px] border-t border-[#14212e]/14 pt-5"><ProjectBack /></div></section>
    </main>
  )
}
