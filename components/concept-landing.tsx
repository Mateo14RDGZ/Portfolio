...
function BrumaLanding() {
  const menu = [
    ['Espresso de la casa', 'Cacao, nuez y caramelo', '$ 150'],
    ['Filtro de temporada', 'Notas florales y cítricas', '$ 190'],
    ['Flat white', 'Doble espresso y leche texturada', '$ 180'],
    ['Tostada de pan madre', 'Manteca batida y conserva', '$ 240'],
  ]
  const faqs = [
    ...
    <main className="overflow-x-clip bg-[#edf0df] text-[#23201b] [font-family:var(--font-fraunces),Georgia,serif]">
      ...
      <header className="border-b border-[#23201b]/15 bg-[#edf0df]">
        <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
          <Link href="#inicio" className="flex items-center gap-3 text-[1.7rem] leading-none tracking-[-0.06em]"><span className="grid size-9 place-items-center rounded-full border border-[#23201b]/35"><Coffee className="size-4" /></span>Bruma</Link>
      ...
      <section id="inicio" className="mx-auto grid max-w-[1440px] lg:grid-cols-[0.82fr_1.18fr]">
        ...
        <figure className="relative min-h-[420px] overflow-hidden bg-[#343027] lg:min-h-0"><Image src="/concepts/bruma-hero.webp" alt="Taza de café de especialidad sobre una barra de piedra" fill priority sizes="(max-width:1023px) 100vw, 62vw" className="object-cover" /><figcaption className="absolute right-5 bottom-5 left-5 flex justify-between border-t border-white/35 pt-3 text-sm text-white"><span>La primera taza del día.</span><span>08:30 - 18:00</span></figcaption></figure>
      </section>

      <section id="origen" className="border-y border-[#23201b]/15 px-5 py-20 sm:px-8 sm:py-28 lg:px-12">
        <div className="mx-auto grid max-w-[1220px] gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
          <Reveal><p className="text-[11px] tracking-[0.18em] uppercase text-[#766953]">El origen</p><h2 className="mt-5 max-w-md text-[clamp(3.2rem,6vw,6.4rem)] leading-[0.84] tracking-[-0.075em]">El café cuenta de dónde viene.</h2></Reveal>
      ...
      <section id="carta" className="bg-[#c9c2a4] px-5 py-20 sm:px-8 sm:py-28 lg:px-12">
        <div className="mx-auto max-w-[1220px]"><Reveal><h2 className="max-w-2xl text-[clamp(3.4rem,6vw,6.5rem)] leading-[0.82] tracking-[-0.075em]">La carta de todos los días.</h2></Reveal><div className="mt-12 grid gap-x-12 border-t border-[#23201b]/25 sm:grid-cols-2">{menu.map(([name, description, price], index) => <Reveal key={name} delay={index * 0.04} className="grid grid-cols-[1fr_auto] gap-x-4 border-b border-[#23201b]/25 py-6"><div><h3 className="text-2xl italic">{name}</h3><p className="mt-2 text-sm text-[#4e493e]">{description}</p></div><span className="text-sm">{price}</span></Reveal>)}</div><p className="mt-7 text-sm text-[#4e493e]">También hay pastelería de estación y alternativas vegetales. Preguntá qué salió hoy.</p></div>
      </section>

      <section className="mx-auto grid max-w-[1440px] gap-7 px-5 py-20 sm:px-8 sm:py-28 lg:grid-cols-[1.12fr_0.88fr] lg:px-12"><Reveal className="relative aspect-[1.1] overflow-hidden bg-[#453c30] sm:aspect-[1.5]"><Image src="/concepts/bruma-cafe-editorial.png" alt="Barista preparando café en una barra de Bruma" fill sizes="(max-width:1023px) 100vw, 55vw" className="object-cover" /></Reveal><Reveal delay={0.08} className="flex flex-col justify-center py-4 lg:px-12"><h2 className="text-[clamp(3.1rem,5vw,5.8rem)] leading-[0.84] tracking-[-0.075em]">Una rutina que se vuelve ritual.</h2><p className="mt-6 max-w-md text-lg leading-relaxed text-[#4e493e]">Barra, mesa o ventana. La experiencia está pensada para que puedas quedarte el rato que precisás.</p><p className="mt-9 text-xl italic">“El lugar donde empezamos los sábados.”</p><span className="mt-2 text-sm text-[#605a4c]">Lucía y Santiago, vecinos</span></Reveal></section>

      <section className="border-y border-[#23201b]/15 px-5 py-20 sm:px-8 sm:py-28 lg:px-12"><div className="mx-auto grid max-w-[1220px] gap-12 lg:grid-cols-[0.75fr_1.25fr]"><Reveal><h2 className="text-[clamp(3rem,5vw,5.7rem)] leading-[0.85] tracking-[-0.075em]">Preguntas que aparecen antes de venir.</h2></Reveal><div>{faqs.map(([question, answer]) => <details key={question} className="group border-t border-[#23201b]/20 py-5 last:border-b"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-xl"><span>{question}</span><ChevronDown className="size-5 transition-transform duration-300 group-open:rotate-180" /></summary><p className="max-w-xl pt-4 leading-relaxed text-[#4e493e]">{answer}</p></details>)}</div></div></section>

      <section id="visita" className="bg-[#29261f] px-5 py-20 text-[#edf0df] sm:px-8 sm:py-28 lg:px-12"><div className="mx-auto grid max-w-[1220px] gap-10 lg:grid-cols-[1fr_auto] lg:items-end"><Reveal><p className="text-[11px] tracking-[0.18em] uppercase text-[#c9c2a4]">Bruma Café</p><h2 className="mt-5 max-w-3xl text-[clamp(3.7rem,6.6vw,7.2rem)] leading-[0.8] tracking-[-0.08em]">Pasá cuando necesites una pausa.</h2></Reveal><Reveal delay={0.08} className="space-y-3 text-base text-[#edf0df]/75"><p className="flex gap-3"><MapPin className="mt-0.5 size-4" />Ciudad Vieja, Montevideo</p><p>Lunes a sábado, 08:30 - 18:00</p><Link href="/#contact" className="mt-6 inline-flex min-h-12 items-center gap-3 border border-[#edf0df]/45 px-5 text-[#edf0df]">Quiero una web así <ArrowUpRight className="size-4" /></Link></Reveal></div><div className="mx-auto mt-14 max-w-[1220px] border-t border-[#edf0df]/20 pt-5"><ProjectBack /></div></section>
    </main>
  )
}

function LineaNorteLanding() {
  const vehicles = [
    ['Arco E-9', 'SUV eléctrico', '612 km de autonomía estimada'],
    ['Norte S1', 'Gran turismo', 'Tracción total y 4 plazas'],
    ['Vector 4', 'SUV familiar', 'Espacio, tecnología y carga rápida'],
  ]
  const faq = [['¿Aceptan usados?', 'Tomamos tu vehículo como parte de pago después de una evaluación presencial.'], ['¿Hay financiación?', 'Las alternativas dependen de la unidad y se simulan antes de avanzar con la reserva.'], ['¿Puedo coordinar una prueba?', 'Sí. Elegís una unidad, nos contás tu disponibilidad y confirmamos el recorrido.']]
  return (
