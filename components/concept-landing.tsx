...
function AsterLanding() {
  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-[#080a14] text-[#eff3ff] [font-family:var(--font-space-grotesk),Arial,sans-serif]">
      ...
      <section className="border-t border-white/10 px-5 py-16 sm:px-8 lg:px-12"><div className="mx-auto max-w-[1240px]"><Reveal><p className="font-mono text-[11px] tracking-[0.18em] text-[#8cf1ff] uppercase">Ficha técnica / 04</p><h2 className="mt-3 text-3xl tracking-[-.05em]">Preguntas frecuentes</h2></Reveal><div className="mt-7 border-t border-white/10">{faq.map(([question, answer], index) => <Reveal key={question} delay={index * 0.05} className="grid gap-1.5 border-b border-white/10 py-5 sm:grid-cols-[minmax(0,15rem)_1fr] sm:gap-8"><h3 className="font-mono text-sm text-[#dce6ff]">{question}</h3><p className="text-sm leading-relaxed text-[#aebbd8]">{answer}</p></Reveal>)}</div><div className="mt-10 border-t border-white/10 pt-5"><ProjectBack className="text-[#dce6ff]" /></div></div></section>
    </main>
  )
}
